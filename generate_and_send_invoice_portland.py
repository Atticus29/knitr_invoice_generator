...
PDX_CALENDAR_ID = os.getenv("PDX_CALENDAR_ID")
if not PDX_CALENDAR_ID:
	raise ValueError("PDX_CALENDAR_ID environment variable is required")
...
def main(year: int = None, month: int = None):
	...
	events = fetch_events_for_month(service, PDX_CALENDAR_ID, year, month)
	if not events:
	...
	csv_name = f"sbg_portland_invoice_{invoice_date.day}_{month_name}_{invoice_date.year}.csv"
	pdf_name = f"sbg_portland_invoice_{invoice_date.day}_{month_name}_{invoice_date.year}.pdf"
...
def parse_arguments():
	...
	parser = argparse.ArgumentParser(
		...
		epilog="""
Examples:
  python generate_and_send_invoice_portland.py                    # Current month
  python generate_and_send_invoice_portland.py -m 10              # October of current year
  python generate_and_send_invoice_portland.py -m 10 -y 2024      # October 2024
  python generate_and_send_invoice_portland.py --month 12 --year 2023  # December 2023
		"""
	)
